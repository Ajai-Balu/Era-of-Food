...
public class UserDataDAO {
    ...
    public void insertData(UserDataBO userBO) throws SQLException {
        ...
        try {
        	...
        	String query1 = "SELECT email FROM eof.USER_DATA WITH (NOLOCK) WHERE email = ? and is_delete = 0";
    ...
    public void validate(UserDataBO userBO) throws SQLException{
        ...
        try {
            ...
            String query = "SELECT user_id,email,password,name,role FROM eof.USER_DATA WITH (NOLOCK)  WHERE email = ? and password = ? and is_delete = 0";
    ...
    public JSONObject getUserDetails(UserDataBO userBO) throws SQLException{
        ...
        try {
            ...
            String query = "SELECT USR.user_id,name,age,email,gender,phone_number,likes,ISNULL(RESP.count,0) as count FROM eof.USER_DATA USR with (NOLOCK) left join (SELECT COUNT(recipe_id) as count,user_id FROM EOF.RECIPE_MASTER with (NOLOCK) where is_delete = 0 group by user_id) RESP ON RESP.user_id = USR.user_id WHERE USR.user_id = ? and is_delete = 0";
            pstmt = con.prepareStatement(query);
    ...
    public void updateUserDetails(UserDataBO userBO) throws SQLException{
        ...
        try {
        	...
        	String query = "SELECT user_id FROM eof.USER_DATA WITH (NOLOCK) WHERE user_id = ? and password = ? and is_delete = 0";
            pstmt = con.prepareStatement(query);
            pstmt.setInt(1, userBO.getUser_id());
            pstmt.setString(2, userBO.getPassword());
            rSet = pstmt.executeQuery();
    ...
    public JSONArray getUsers() throws SQLException{
		...
		try {
            ...
            String query = "SELECT USR.user_id,name,age,email,gender,phone_number,likes,ISNULL(RESP.count,0) as count FROM eof.USER_DATA USR with (NOLOCK) left join (SELECT COUNT(recipe_id) as count,user_id FROM EOF.RECIPE_MASTER with (NOLOCK) where is_delete = 0 group by user_id) RESP ON RESP.user_id = USR.user_id WHERE USR.role = 'user' and is_delete = 0";
            pstmt = con.prepareStatement(query);
            rSet = pstmt.executeQuery();
            while (rSet.next()) { 
    ...
    public JSONObject getCounts() throws SQLException{
        ...
        try {
        	...
        	query.append("SELECT COUNT(user_id) as user_count FROM eof.USER_DATA with (NOLOCK) WHERE is_delete = 0 and role = 'user'");
            ...
            String qry = "SELECT COUNT(recipe_id) as recipe_count FROM eof.RECIPE_MASTER with (NOLOCK) WHERE is_delete = 0";
            pstmt1 = con.prepareStatement(qry);
